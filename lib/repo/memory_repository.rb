...
module Repository
  ...
  class MemoryRepository < Repository::AbstractRepository
    ...
    def initialize(location)
      ...
      @timestamps_revisions = {}
      @timestamps_revisions[Time.now] = @current_revision   # push first timestamp-revision mapping
    ...
    def get_revision_number_by_timestamp(wanted_timestamp, path = nil)
      ...
      @timestamps_revisions.keys().each do |time_dump|
        all_timestamps_list.push(time_dump)
        remaining_timestamps_list.push(time_dump)
      end
      ...
      all_timestamps_list.each do |best_match|
        ...
        if path.nil? || (!path.nil? && @timestamps_revisions[best_match].revision_at_path(path))
      ...
      remaining_timestamps_list.each do |curr_timestamp|
        ...
        if path.nil? || (!path.nil? && @timestamps_revisions[curr_timestamp].revision_at_path(path))
      ...
      if first_timestamp_found
        ...
        return @timestamps_revisions[wanted_timestamp]
      else
        return @current_revision
      end
    end
